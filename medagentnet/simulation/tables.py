...
def generate_all_tables(results: dict) -> str:
    ...
    parts = ["% Generated by simulation/tables.py from a single results.json.",
             ...
             f"% Patients: {meta.get('num_patients', '')}  "
             f"seed: {meta.get('base_seed', '')}  "
             f"concurrency: {meta.get('concurrency', 1)}", ""]
    if meta.get("concurrency", 1) > 1:
        parts.append("% NOTE: this run was parallel. Latency rows include "
                     "queueing at the model server and are NOT per-request "
                     "latency. Take reported latency from E7, whose patient and "
                     "department sweeps are pinned to concurrency 1.")
        parts.append("")
    builders = [
